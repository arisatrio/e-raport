...
<x-page-header>
    ...
    @slot('breadcrumb')
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item">Edit Tahun Ajaran</li>
        </ol>
    @endslot
</x-page-header>
<x-page-content>
    ...
    @slot('content')
        <form class="form-horizontal" id="form-upload-preview" method="POST" action="{{ route('admin.tahun-ajaran.update', $ta->id) }}">
            @csrf
            @method('PUT')
            <div class="form-group row">
                <label for="email1" class="col-sm-2 control-label col-form-label">Tahun Ajaran</label>
                <input type="text" class="form-control col-sm-10" name="tahun_ajaran" value="{{$ta->tahun_ajaran}}" required>
            </div>
